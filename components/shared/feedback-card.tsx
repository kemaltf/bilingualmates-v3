...
function iconByStatus(status: FeedbackStatus) {
  if (status === "correct") return <CheckCircle2 className="size-6 text-emerald-600" />
  if (status === "incorrect") return <XCircle className="size-6 text-rose-600" />
...
export function FeedbackCard({ status, title, message, explanation, media, className }: FeedbackCardProps) {
  return (
    <Card className={cn(containerClasses(status), className)}>
      <CardHeader className="flex flex-row items-start gap-3">
        ...
        <div className="flex-1 space-y-1">
          ...
          {message && <CardDescription className="text-neutral-700">{message}</CardDescription>}
        </div>
      </CardHeader>
      <CardContent className="space-y-2">
        {explanation && <div className="text-sm text-neutral-600">{explanation}</div>}
        {media && (
          <div className="mt-1">
