...
export function FeaturedMissionCard({
  ...
  return (
    <div
      className={cn(
        "bg-white rounded-2xl shadow p-4 border-[3px] border-slate-300",
        className
      )}
    >
        ...
        <div className="md:col-span-2">
          <div className="relative w-full aspect-video rounded-2xl bg-slate-200 overflow-hidden">
            ...
            {video && (
              <div className="absolute bottom-2 left-2 flex items-center gap-2">
                <span className="px-2 py-1 rounded-full text-xs font-bold bg-black/80 text-white">
          ...
          {video && (
            <div className="mt-3 flex items-center gap-2">
              <span className="px-2 py-1 rounded-full text-xs font-bold bg-slate-200 text-slate-900 capitalize">
                {video.difficulty}
              </span>
        ...
        <div className="flex flex-col justify-between">
          <div>
            <div className="text-xl font-bold">
              {video ? video.title : "Select a world"}
            </div>
          </div>
          <div className="flex justify-end mt-3">
            <Button
              ...
              size="md"
              onClick={onStart}
              label={progressPercent > 0 ? "RESUME" : "START"}
            />
          </div>
        </div>
      </div>
