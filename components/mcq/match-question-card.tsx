...
function isItemMatched(id: string, matches: { leftId: string; rightId: string }[]) {
  return matches.some((m) => m.leftId === id || m.rightId === id)
}
...
function MatchButton({
  ...
  const variant = matched ? feedbackVariant ?? "option-disabled" : selected ? "option-selected" : "option-default"
  return (
    <OptionButton
      variant={variant}
      disabled={matched}
      ...
      label={label}
      showLabel={showLabel}
      className={cn("w-full h-auto justify-start text-left rounded-2xl px-4 py-3")}
    >
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)}>
      <div className={cn("w-full", questionClassName)}>
        ...
        {question.textPrompt && <p className="mt-2 text-base">{question.textPrompt}</p>}
      </div>

      <div className={cn("grid gap-4", containerClassName)}>
        <div className="flex flex-col gap-3">
          {leftTitle && <div className="text-sm font-semibold uppercase tracking-wide text-neutral-600">{leftTitle}</div>}
          {leftItems.map((item, idx) => {
            const matchedPair = matches.find((m) => m.leftId === item.id)
            const matched = !!matchedPair
            const hasCorrect = Array.isArray(question.correctPairs) && question.correctPairs.length > 0
            const isCorrect = hasCorrect && !!matchedPair && question.correctPairs!.some((p) => p.leftId === matchedPair.leftId && p.rightId === matchedPair.rightId)
            ...
            return (
              <MatchButton
                ...
                selected={selected}
                matched={matched}
                feedbackVariant={matched && hasCorrect ? (isCorrect ? "option-correct" : "option-incorrect") : undefined}
                onClick={() => {
                  if (matched) return
        ...
        <div className="flex flex-col gap-3">
          {rightTitle && <div className="text-sm font-semibold uppercase tracking-wide text-neutral-600">{rightTitle}</div>}
