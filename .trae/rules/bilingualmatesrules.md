...
4.  **Centralized Helpers**: If a utility function or helper is used in more than one place (or expected to be), define it in a centralized file (e.g., `lib/utils.ts` or `lib/formatters.ts`) instead of duplicating it or keeping it inside a component file.
...
4.  **UI Internationalization**: All user-facing UI text MUST be implemented using `next-intl` and provided in both English (`messages/en.json`) and Indonesian (`messages/id.json`).
